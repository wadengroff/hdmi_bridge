...
always_comb begin
    ...
    if (risedge_controller_s && i2c_scl_p == 1) begin
        ...
        case (i2c_state_reg_s)
            ...
            IDLE: begin
                if (i2c_scl_p == 1 && falledge_controller_s) begin
                    i2c_state_s = ADDR;
                    bit_ind_s = 0;
                end
            end
            ...
            ADDR: begin
                ...
                if (risedge_scl_s) begin
                    ...
                    i2c_address_s = {i2c_address_reg_s[5:0], controller_sda_p};
            ...
            RW_EN: begin
                ...
                if (risedge_scl_s) begin
                    ...
                    rd_en_s = controller_sda_p;
                    wr_en_s = ~controller_sda_p;
                end else if (falledge_scl_s) begin
            ...
            RDATA: begin
                if (risedge_scl_s) begin
                    bit_ind_s = bit_ind_reg_s + 1;
                    rdata_s = {rdata_reg_s[6:0], subordinate_sda_p};
                end else if (falledge_scl_s) begin
            ...
            WDATA: begin
                if (risedge_scl_s) begin
                    bit_ind_s = bit_ind_reg_s + 1;
                    wdata_s = {wdata_reg_s[6:0], controller_sda_p};
                end else if (falledge_scl_s) begin
                    if (bit_ind_reg_s == 8) begin
                        i2c_state_s = ACK;
                    end
...
always_ff @(posedge clk_p) begin
    ...
    controller_sda_reg_s <= controller_sda_p;
    risedge_controller_s <= (controller_sda_reg_s == 0 && controller_sda_p == 1) ? 1 : 0;
    falledge_controller_s <= (controller_sda_reg_s == 1 && controller_sda_p == 0) ? 1 : 0;

    // detect edges on subordinate-side sda
    subordinate_sda_reg_s <= subordinate_sda_p;
    risedge_subordinate_s <= (subordinate_sda_reg_s == 0 && subordinate_sda_p == 1) ? 1 : 0;
    falledge_subordinate_s <= (subordinate_sda_reg_s == 1 && subordinate_sda_p == 0) ? 1 : 0;
...
always_comb begin
    case (i2c_state_reg_s)
        ...
        ADDR: begin
            ...
            subordinate_sda_ten_p = controller_sda_p;      // Output the data from controller
        end
        RW_EN: begin
            controller_sda_ten_p = 1;                      // Need data from controller, so tristate outptu
            subordinate_sda_ten_p = controller_sda_p;
        end
        START_ACK: begin
            controller_sda_ten_p = subordinate_sda_p;      // Output data from subordinate
        ...
        ACK: begin
            if (wr_en_reg_s) begin
                ...
                controller_sda_ten_p = 1;                  // Need data from controller, so tristate output
                subordinate_sda_ten_p = controller_sda_p;  // Output data from controller
            end
        end
        RDATA: begin
            controller_sda_ten_p = subordinate_sda_p;      // Output data from subordinate
